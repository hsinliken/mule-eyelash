             ...
             {appointments.map(apt => (
               <div key={apt.id} className="bg-white p-4 rounded-xl shadow-sm border border-brand-100">
                  <div className="flex justify-between items-start mb-3">
                     <div>
                       <span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider mb-2 ${
        ...
        {activeTab === 'settings' && (
           <div className="animate-fade-in bg-white rounded-xl shadow-lg border border-brand-200 p-6">
              ...
              <div className="space-y-6">
                 <div>
                    <label className="block text-xs font-bold text-brand-500 uppercase mb-2">商家 LINE ID (官方帳號)</label>
                    <input 
                       type="text" 
                       value={settingsForm.lineId} 
                       onChange={e => setSettingsForm({...settingsForm, lineId: e.target.value})}
                       placeholder="例如: @mule_eyelash"
                       className="w-full bg-brand-50 border border-brand-200 rounded-lg p-3 text-sm"
                    />
                    <p className="text-[10px] text-brand-400 mt-1">用於「線上諮詢」按鈕，請包含 @ 符號。</p>
                 </div>
                          ...
                          <div className="flex justify-between items-start border-b border-brand-50 pb-3 mb-3">
                            ...
                            <div className="text-right flex flex-col items-end gap-1">
                                ...
                                <span className={`text-[10px] px-2 py-0.5 rounded border ${
                                  order.payment.isPaid ? 'border-green-200 text-green-600 bg-green-50' : 'border-red-200 text-red-500 bg-red-50'
                                }`}>
